...
const classVariants = {
    ...
    reset: "bg-gray-200 hover:bg-gray-300 translate-y-1",
...
function SketchyButton({ className, variant = "play", onClick }: Props) {
    return (
        <button id="playPauseBtn" className={`text-gray-800 font-medium text-sm transition-colors duration-300 p-0.5 active:animate-bounce-sketch cursor-pointer ${classVariants[variant]} ${className}`} onClick={onClick}>
            {icons[variant]}
        </button>
    )
}
